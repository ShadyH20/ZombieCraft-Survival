...
void GLTexture::Load(char *name)
{
	...
	if(strstr(texturename, ".tga"))	
		LoadTGA(texturename);
	else if (strstr(texturename, ".png"))  // Added PNG support
		LoadPNG(texturename);
}
...
// Add the following LoadPNG method
void GLTexture::LoadPNG(char* name) {
	...
	// check the file extension to see if it's a PNG
	if (strstr(texturename, ".png")) {
		// Load the PNG file into a buffer
		std::vector<unsigned char> buffer;
		unsigned w, h;

		// Use lodepng to decode the PNG file directly into buffer
		unsigned error = lodepng::decode(buffer, w, h, texturename);

		if (!error) {
			// Allocate the memory for the image data
			GLubyte* imageData = new GLubyte[buffer.size()];
			std::copy(buffer.begin(), buffer.end(), imageData);

			// Set the type (assuming RGBA for PNG)
			GLenum type = GL_RGBA;

			// Set other texture parameters if needed
			// ...
			...
			// Generate the mipmaps
			gluBuild2DMipmaps(GL_TEXTURE_2D, type, w, h, type, GL_UNSIGNED_BYTE, imageData);
		...
		// Handle other texture formats or display an error message
		std::cerr << "Unsupported texture format: " << texturename << std::endl;
	}
}
